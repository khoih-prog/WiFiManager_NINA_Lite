...
#error EPROM_START + CONFIG_DATA_SIZE > EEPROM_SIZE. Please adjust.
    ...
    {   
      ...
      String WIFININA_HTML_SCRIPT    = "<script id=\"jsbin-javascript\">function udVal(key,val){var request=new XMLHttpRequest();var url='/?key='+key+'&value='+val;request.open('GET',url,false);request.send(null);}\
